...
def render_and_save_streaming(
    ...
    """
    Memory-efficient streaming render + save.

    Instead of accumulating all rendered frames into RAM and then writing video,
    this function renders one timestamp at a time (num_cams frames), tiles them,
    writes the video frame immediately, and discards the pixel data.

    Peak RAM ~ num_cams × num_channels × frame_size (order of MB, not GB).
    Returns only scalar metrics; no image arrays are included in the return value.
    A ``vis_frames`` key holds one sample frame per key for Weights & Biases logging.
    """
    if keys is None:
        keys = ["gt_rgbs", "rgbs", "depths"]

    trainer.set_eval()

    # ---- scalar metric accumulators (tiny memory) ----
    if compute_metrics:
        psnrs, ssim_scores, lpipss = [], [], []
        masked_psnrs, masked_ssims = [], []
        human_psnrs, human_ssims = [], []
    ...
    return_frame_dict: Dict[str, np.ndarray] = {}

    def _make_writer(path: str):
        os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
        if num_timestamps <= 1:
            return imageio.get_writer(path, mode="I")
        return imageio.get_writer(path, mode="I", fps=fps)

    if save_seperate_video:
        writers = {
            key: _make_writer(video_save_pth.replace(".mp4", f"_{key}.mp4"))
            for key in keys
        }
    else:
        cat_writer = _make_writer(video_save_pth)

    # ---- per-timestamp frame buffer ----
    # ts_buf[channel_name] = list of num_cams numpy arrays for the current timestamp
    ts_buf: Dict[str, list] = {}
    cam_names_buf: List[str] = []
    ts_idx = 0

    green_bg = torch.tensor([0.0, 177, 64]) / 255.0

    def _buf(key: str, arr: np.ndarray):
        ts_buf.setdefault(key, []).append(arr)

    def _prepare_frames(key: str):
        """Return a list of visualisable frames for one key (one timestamp)."""
        frames = ts_buf.get(key, [])
        if not frames:
            return None
        if key == "gt_sky_masks":
            return [np.stack([f, f, f], axis=-1) for f in frames]
        elif "mask" in key:
            opacity_key = key.replace("mask", "opacities")
            if opacity_key not in ts_buf:
                return None
            return [np.stack([f, f, f], axis=-1) for f in ts_buf[opacity_key]]
        elif "depth" in key:
            opacity_key = key.replace("depths", "opacities")
            if opacity_key not in ts_buf:
                return None
            return [depth_visualizer(f, op) for f, op in zip(frames, ts_buf[opacity_key])]
        return frames

    def _flush_timestamp():
        """Tile and write one complete timestamp's buffered frames, then clear."""
        nonlocal ts_idx
        cam_names = list(cam_names_buf)

        if save_seperate_video:
            for key in keys:
                frames = _prepare_frames(key)
                if frames is None:
                    continue
                tiled = layout(frames, cam_names)
                frame_u8 = to8b(tiled)
                writers[key].append_data(frame_u8)
                if ts_idx == return_frame_id:
                    return_frame_dict[key] = frame_u8
        else:
            merged = []
            for key in keys:
                frames = _prepare_frames(key)
                ...
                merged.append(tiled)
            if merged:
                merged_frame = to8b(np.concatenate(merged, axis=0))
                cat_writer.append_data(merged_frame)
                if ts_idx == return_frame_id:
                    return_frame_dict["concatenated_frame"] = merged_frame
    ...
    for idx in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
        image_infos, cam_infos = dataset.get_image(idx, camera_downscale)
        for k, v in image_infos.items():
            if isinstance(v, Tensor):
                image_infos[k] = v.cuda(non_blocking=True)
        for k, v in cam_infos.items():
        ...
        rgb = results["rgb"]
        green_bg_dev = green_bg.to(rgb.device)

        # ---- fill per-key buffers ----
        _buf("rgbs", get_numpy(rgb))
        if "pixels" in image_infos:
            _buf("gt_rgbs", get_numpy(image_infos["pixels"]))

        for node in ["Background", "RigidNodes", "DeformableNodes", "SMPLNodes", "Dynamic"]:
            rgb_key = f"{node}_rgb"
            if rgb_key in results:
                blended = (
                    results[rgb_key] * results[f"{node}_opacity"]
                    + green_bg_dev * (1 - results[f"{node}_opacity"])
                )
                _buf(f"{node}_rgbs", get_numpy(blended))
            depth_key = f"{node}_depth"
            if depth_key in results:
                _buf(f"{node}_depths", get_numpy(results[depth_key]))
                _buf(f"{node}_opacities", get_numpy(results[f"{node}_opacity"]))

        if compute_error_map and "pixels" in image_infos:
            err = (rgb - image_infos["pixels"]) ** 2
            err = err.mean(dim=-1, keepdim=True)
            err = (err - err.min()) / (err.max() - err.min())
            err = err.repeat_interleave(3, dim=-1)
            _buf("rgb_error_maps", get_numpy(err))

        if "rgb_sky_blend" in results:
            _buf("rgb_sky_blend", get_numpy(results["rgb_sky_blend"]))
        if "rgb_sky" in results:
            _buf("rgb_sky", get_numpy(results["rgb_sky"]))

        _buf("depths", get_numpy(results["depth"]))
        if "opacity" in results:
            _buf("opacities", get_numpy(results["opacity"]))
        if "sky_masks" in image_infos:
            _buf("gt_sky_masks", get_numpy(image_infos["sky_masks"]))
        ...
        if "lidar_depth_map" in image_infos:
            ...
            mask = (depth_map.unsqueeze(-1) > 0).cpu().numpy()
            lidar_on_image = (
                image_infos["pixels"].cpu().numpy() * (1 - mask) + depth_img * mask
            )
            _buf("lidar_on_images", lidar_on_image)

        cam_names_buf.append(cam_infos["cam_name"])

        # ---- per-frame metrics ----
        if compute_metrics and "pixels" in image_infos:
            psnr = compute_psnr(rgb, image_infos["pixels"])
            ssim_score = ssim(
                get_numpy(rgb),
                get_numpy(image_infos["pixels"]),
            ...
            if "sky_masks" in image_infos:
                ...
                if occupied_mask.sum() > 0:
                    occupied_psnrs.append(
                        compute_psnr(rgb[occupied_mask], image_infos["pixels"][occupied_mask])
                    )
                    occupied_ssims.append(
                        ssim(
                            get_numpy(rgb),
            ...
            if "dynamic_masks" in image_infos:
                dyn_mask = get_numpy(image_infos["dynamic_masks"]).astype(bool)
                if dyn_mask.sum() > 0:
                    masked_psnrs.append(
                        compute_psnr(rgb[dyn_mask], image_infos["pixels"][dyn_mask])
                    )
                    masked_ssims.append(
                        ssim(
                            get_numpy(rgb),
            ...
            if "human_masks" in image_infos:
                h_mask = get_numpy(image_infos["human_masks"]).astype(bool)
                if h_mask.sum() > 0:
                    human_psnrs.append(
                        compute_psnr(rgb[h_mask], image_infos["pixels"][h_mask])
                    )
                    human_ssims.append(
                        ssim(
                            get_numpy(rgb),
            ...
            if "vehicle_masks" in image_infos:
                v_mask = get_numpy(image_infos["vehicle_masks"]).astype(bool)
                if v_mask.sum() > 0:
                    vehicle_psnrs.append(
                        compute_psnr(rgb[v_mask], image_infos["pixels"][v_mask])
                    )
                    vehicle_ssims.append(
                        ssim(
                            get_numpy(rgb),
        ...
        # ---- flush after each complete timestamp ----
        if len(cam_names_buf) == num_cams:
            _flush_timestamp()

    # flush any partial last timestamp
    if cam_names_buf:
        _flush_timestamp()
    ...
    if save_seperate_video:
        for key, w in writers.items():
            w.close()
            if verbose:
                logger.info(f"saved video to {video_save_pth.replace('.mp4', f'_{key}.mp4')}")
    else:
        cat_writer.close()
        if verbose:
            logger.info(f"saved video to {video_save_pth}")

    # ---- aggregate and log metrics ----
    results_dict: Dict = {
        "psnr": non_zero_mean(psnrs) if compute_metrics else -1,
        "ssim": non_zero_mean(ssim_scores) if compute_metrics else -1,
        "lpips": non_zero_mean(lpipss) if compute_metrics else -1,
        "occupied_psnr": non_zero_mean(occupied_psnrs) if compute_metrics else -1,
        "occupied_ssim": non_zero_mean(occupied_ssims) if compute_metrics else -1,
        "masked_psnr": non_zero_mean(masked_psnrs) if compute_metrics else -1,
        "masked_ssim": non_zero_mean(masked_ssims) if compute_metrics else -1,
        "human_psnr": non_zero_mean(human_psnrs) if compute_metrics else -1,
        "human_ssim": non_zero_mean(human_ssims) if compute_metrics else -1,
        "vehicle_psnr": non_zero_mean(vehicle_psnrs) if compute_metrics else -1,
        "vehicle_ssim": non_zero_mean(vehicle_ssims) if compute_metrics else -1,
    }

    if compute_metrics:
        num_samples = len(indices) if vis_indices is not None else len(dataset)
        logger.info(f"Eval over {num_samples} images:")
        logger.info(f"\t Full Image  PSNR: {results_dict['psnr']:.4f}")
        logger.info(f"\t Full Image  SSIM: {results_dict['ssim']:.4f}")
        logger.info(f"\t Full Image LPIPS: {results_dict['lpips']:.4f}")
        logger.info(f"\t     Non-Sky PSNR: {results_dict['occupied_psnr']:.4f}")
        logger.info(f"\t     Non-Sky SSIM: {results_dict['occupied_ssim']:.4f}")
        logger.info(f"\tDynamic-Only PSNR: {results_dict['masked_psnr']:.4f}")
        logger.info(f"\tDynamic-Only SSIM: {results_dict['masked_ssim']:.4f}")
        logger.info(f"\t  Human-Only PSNR: {results_dict['human_psnr']:.4f}")
        logger.info(f"\t  Human-Only SSIM: {results_dict['human_ssim']:.4f}")
        logger.info(f"\tVehicle-Only PSNR: {results_dict['vehicle_psnr']:.4f}")
        logger.info(f"\tVehicle-Only SSIM: {results_dict['vehicle_ssim']:.4f}")

    results_dict["vis_frames"] = return_frame_dict
    return results_dict
